...
let displayBanner = async (): Promise<void> => {
    showBanner(
        "Word Counter",
        "You can count number of words by pasting or writing below."
    );
};

// Function to prompt the user for a paragraph of text
let paragraph = async (): Promise<string> => {
    ...
    let input: string = await userPara;
    return input;
};

// Function to count the words and letters in the given text
let wordsAndLettersCounter = async (para: string): Promise<void> => {
...
let confirmExit = async (): Promise<boolean> => {
...
(async () => {
    ...
    while (true) {
        let input: string = await paragraph();
